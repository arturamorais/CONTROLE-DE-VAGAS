...
Sistema web completo para gerenciamento de solicitações de vagas escolares. Permite que responsáveis solicitem vagas para seus filhos e que a equipe administrativa avalie, aprove ou reprove cada solicitação.
...
- Visão geral com totais por status
- Gestão completa de solicitações (pendente → em análise → aprovado/reprovado)
...
- Enturmar: criação de turmas e alocação de alunos aprovados
- Relatório analítico com gráficos (status, segmentos, séries, turnos, evolução mensal, motivos)
...
O sistema depende de políticas RLS configuradas no Supabase para garantir que:
...
Consulte a seção de segurança abaixo para o SQL completo.
...
-- Políticas (ver documentação completa no código)
```
...
Após criar uma conta normalmente, insira o registro via SQL:
```sql
